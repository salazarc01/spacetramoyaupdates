...
export const generateTimedMessage = async (type: 'morning' | 'evening'): Promise<string> => {
  // Inicialización bajo demanda para mayor seguridad y evitar errores en boot
  if (!process.env.API_KEY) {
    return type === 'morning' ? "¡Buenos días, bellezas! Es hora de conquistar la pasarela virtual." : "¡Buenas noches! Que los sueños de corona se hagan realidad.";
  }
